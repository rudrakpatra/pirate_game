...
export class EventSystem {
  ...
  pushShootEvent(shipIndex: number, offset: Vector2D, delay: number): void {
    ...
    const fireTime =
      Math.ceil((currentTime + delay) / this.TriggerPeriod) *
      this.TriggerPeriod;
  ...
  pushTurnEvent(shipIndex: number, rate: number): void {
    const currentTime = Date.now() / 1000;
    const applyTime =
      Math.ceil(currentTime / this.TriggerPeriod) * this.TriggerPeriod;
  ...
  update(deltaTime: number): void {
    ...
    const shootEventsToFire = this.shootEvents.filter(
      (event) => event.fireTime <= currentTime
    );
    shootEventsToFire.forEach((event) => {
      this.ships[event.shipIndex].shoot(event.offset);
    });
    this.shootEvents = this.shootEvents.filter(
      (event) => event.fireTime > currentTime
    );

    // Process turn events
    const turnEventsToApply = this.turnEvents.filter(
      (event) => event.applyTime <= currentTime
    );
    turnEventsToApply.forEach((event) => {
      this.ships[event.shipIndex].applyTurnRateChange(event.rate);
    });
    this.turnEvents = this.turnEvents.filter(
      (event) => event.applyTime > currentTime
    );
  }
}
